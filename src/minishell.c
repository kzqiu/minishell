...
void cd(char *cmd) {
    ...
    if (new_path[0] == '\0' || new_path[0] == '\n') { // Case: go to home dir
        ...
        if (new_path[0] == '~') {
            strcpy(cd_path, usr->pw_dir);
            strcpy(cd_path + strlen(cd_path), new_path + 1);
        } else {
            strcpy(cd_path, new_path);
        }
...
void execute(char *cmd, char *cwd) {
    if (cmd[0] == 'c' && cmd[1] == 'd' && (cmd[2] == ' ' || cmd[2] == '\n')) { // Case 1: cd
        ...
        while (tmp != NULL) {
            if (tmp[0] == '~') {
                if ((tokens[argc] = (char *) malloc(strlen(tmp) + strlen(usr->pw_dir) + 1)) == NULL) {
                    fprintf(stderr, "Error: malloc() failed. %s.\n", strerror(errno));
                    return;
                }
                strcpy(tokens[argc], usr->pw_dir);
                strcpy(tokens[argc] + strlen(tokens[argc]), tmp + 1);
            } else {
